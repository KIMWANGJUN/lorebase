...
import { mockUsers, mockTetrisRankings, tetrisTitles } from '@/lib/mockData'; 
...
const NicknameDisplayForComment: FC<NicknameDisplayForCommentProps> = ({ author, authorDisplayName, postMainCategory }) => {
  ...
  if (isAdminUser) {
    ...
    if (authorRankInPostCategory !== null && authorRankInPostCategory <= 3) {
    ...
    <div className="flex flex-col items-start"> {/* Changed to items-start */}
      ...
      <NicknameWrapper {...wrapperProps}>
        <div className={cn(nicknameContainerClass, "flex items-center gap-1")}>
            {authorHasCategoryPresence && !isAdminUser && !isGlobalTop3 && <CategoryIcon category={postMainCategory} className="h-3.5 w-3.5" />}
...
export default function CommentSection({ postId, initialComments, postMainCategory }: CommentSectionProps) {
  ...
  useEffect(() => {
    const sortedInitialComments = [...initialComments].sort((a,b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()); // Sort ascending for display
  ...
  const handleAddCommentInternal = (content: string, parentId?: string) => {
    ...
    if (parentId) {
      ...
      // Add to top of the list, then re-sort
