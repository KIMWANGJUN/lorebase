  ...
  return (
    <div className="py-8 px-4">
      ...
      <div className="mb-6 flex flex-col md:flex-row gap-4 items-center">
        <DynamicCommunitySearch initialSearchTerm={searchTerm} />
        {user && <CreateButton href={`/community/posts/new?channel=${channelSlug}`} text="새 게시글 작성" icon={PlusCircle} />}
      </div>
        ...
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 bg-card border-border p-1.5 rounded-lg shadow-inner items-center">
          {COMMUNITY_CHANNELS.map((channel) => (
            <TabsTrigger key={channel.slug} value={channel.categories[0]} asChild>
              <Link href={`/community/channels/${channel.slug}`} className="w-full flex items-center justify-center gap-1.5">
